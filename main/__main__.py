...
from config import INPUT_DATA_DIR, OUTPUT_DATA_DIR, make_logger
...
def basis_expansion(data: pd.DataFrame, powers=None) -> pd.DataFrame:
    ...
    """
    logger.info(f'BEGIN: calculate basis function expansions with powers {powers}')
    if not powers:
...
def split_data(data: pd.DataFrame) -> Tuple[pd.DataFrame, pd.DataFrame, pd.DataFrame]:
    ...
    """
    logger.info("BEGIN: split data into training, validation, and test sets.")
...
def run_linear_models_help(train_x: pd.DataFrame, valid_x: pd.DataFrame, test_x: pd.DataFrame,
    ...
    with open(dir / "log.txt", "w") as f:
        ...
        for model in models:
            ...
            f.write(f"validation MSE: {model[3]}\n")
...
def run_linear_models(rw_train_x_bfes: List[pd.DataFrame], rw_valid_x_list: List[pd.DataFrame],  # FIXME Shift F6
    ...
    for i in range(23):
        run_linear_models_help(rw_train_x_bfes[i], rw_valid_x_list[i], rw_test_x_list[i],
